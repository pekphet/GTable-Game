...
object FloorEngine {    //Floor start with 0
    ...
    private fun createMonster(id: Int, floor: Int) = StaticData.getBaseMonster(id).run {
        MonsterData(
                this.baseHP * (1 + floor / 3) + Math.rand(floor / 2),
                this.baseAtk * (1 + floor / 3) + Math.rand(floor / 4),
                this.baseDef * (1 + floor / 3) + Math.rand(floor / 4),
                this.baseExp * (1 + floor / 3) + Math.rand(floor / 2),
    ...
    private fun getGift(g: Gifts, floor: Int) = Gift(g, when (g) {
        ...
        Gifts.HP_RESTORE -> floor + Math.rand(floor)
    })
    ...
    private fun getBuff(buffA: BuffAbility, floor: Int) = Buff(buffA, when (buffA) {
        BuffAbility.ATK -> 1 + Math.rand(floor / 3)
        BuffAbility.DEF -> 1 + Math.rand(floor / 3)
